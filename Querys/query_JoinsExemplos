...
SELECT cus.professional_status, fun.paid_date  
    FROM sales.funnel AS fun
        LEFT JOIN sales.customers AS cus
            ON fun.customer_id = cus.customer_id  -- observe que muitas linhas possuem valor null
        ...
        LEFT JOIN sales.customers AS cus
            ON fun.customer_id = cus.customer_id
                LEFT JOIN temp_tables.ibge_genders AS ibge
                    ON  cus.first_name = ibge.first_name
                        GROUP BY ibge.gender  --DA NULL NO GENDER PQ O = CONSIDERA LETRA MAIUSCULAS 
                        --E MINUSCULAS DE FORMA DIFERENTE, SE USAR ilike GASTA MUITO PROCESSAMENTO
...
SELECT ibge.gender, COUNT(fun.paid_date)
    FROM sales.funnel AS fun
        LEFT JOIN sales.customers AS cus
            ON fun.customer_id = cus.customer_id
                LEFT JOIN temp_tables.ibge_genders AS ibge
                    ON LOWER (cus.first_name) = ibge.first_name -- lower trasnforma todas letras em minusculas
